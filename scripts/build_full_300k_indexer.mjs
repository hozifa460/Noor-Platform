...
// All major Hugging Face dataset files including the massive ones
const TARGET_FILES = [
...
function extractScholar(fileName, raw) {
  ...
  if (fileName.includes('السعودية') || fileName.includes('اللجنة_الدائمة')) return 'اللجنة الدائمة للإفتاء';
  if (fileName.includes('islamweb') || fileName.includes('إسلام_ويب')) return 'موقع إسلام ويب';
...
async function runIndexer() {
  console.log('⚡ Starting Complete 300k+ Fatwa Indexing Pipeline...\n');
  ...
  for (const relativePath of TARGET_FILES) {
    ...
    try {
      ...
      for (let idx = 0; idx < rawRecords.length; idx++) {
        ...
        const title = normalizeTitle(raw.title || raw.question || raw.name || raw.text || raw.Subject || raw.fatwa_title);
        ...
        // Ultra-Lightweight Entry
        const entry = {
          id,
          title: title.slice(0, 140),
          question: question !== title ? question.slice(0, 180) : title.slice(0, 140),
          scholar,
          category,
          tags: tags.slice(0, 3),
