...
// Micro-Index Schema Unification Helper
async function getMicroIndexPayload() {
  const raw = await loadHadithMicroIndex();
  ...
  // Disk fallback if loaded as raw object
  ...
  return { books: [], grades: [], items: [] };
}

function getItemFields(item, payload) {
  if (Array.isArray(item)) {
    const bookIdx = item[0];
    const bookId = typeof bookIdx === 'number' ? (payload?.books?.[bookIdx] || HADITH_BOOKS_LIST[bookIdx]?.id || 'unknown') : String(bookIdx);
    const hadithId = item[1];
    const chapterId = item[2];
    const textPreview = item[3] || '';
    const gradeIdx = item[4];
    const grade = typeof gradeIdx === 'number' ? (payload?.grades?.[gradeIdx] || 'صحيح') : String(gradeIdx || 'صحيح');
    return { bookId, hadithId, chapterId, textPreview, grade };
  } else {
    return {
      bookId: item.b,
      hadithId: item.i,
...
async function runE2ESuite() {
  ...
  await test('T1.4: Micro-index contains >= 50,000 total Hadith items', async () => {
    const payload = await getMicroIndexPayload();
    assertGreaterOrEqual(payload.items.length, 50_000, `Expected >= 50,000 hadiths, got ${payload.items.length}`);
  });

  await test('T1.5: Micro-index items adhere to compact structure [book, id, chapter, text, grade]', async () => {
    const payload = await getMicroIndexPayload();
    assertOk(payload.items.length > 0, 'Index has no items');
    const f = getItemFields(payload.items[0], payload);
    assertOk(f.bookId && f.hadithId && f.textPreview !== undefined, 'Item structure invalid');
  });
  ...
  await test('T1.12: Indexed previews do not begin with raw Isnad chains ("حدثنا...")', async () => {
    const payload = await getMicroIndexPayload();
    const bukhari1 = payload.items.find((item) => {
      const f = getItemFields(item, payload);
      return f.bookId === 'bukhari' && f.hadithId === 1;
    });
    assertOk(bukhari1, 'Bukhari #1 not found in index');
    const f = getItemFields(bukhari1, payload);
    assertOk(!f.textPreview.startsWith('حدثنا الحميدي') && !f.textPreview.startsWith('حدثنا'), 'Isnad chain not stripped');
  });
  ...
  await test('T1.13: Indexed preview retains core prophetic Matn ("إنما الأعمال بالنيات")', async () => {
    ...
    assertOk(bukhari1, 'Bukhari #1 not found in index');
    const f = getItemFields(bukhari1, payload);
    assertOk(f.textPreview.includes('الاعمال بالنيات') || f.textPreview.includes('النيات'), 'Matn missing in index preview');
  });

  await test('T1.14: Previews are cleaned of multiple newlines and carriage returns', async () => {
    const payload = await getMicroIndexPayload();
    for (let i = 0; i < Math.min(50, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertOk(!f.textPreview.includes('\n\n') && !f.textPreview.includes('\r'), `Newline found in item ${i}`);
    }
  });

  await test('T1.15: Indexed previews are compact (<= 450 characters)', async () => {
    const payload = await getMicroIndexPayload();
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertLessOrEqual(f.textPreview.length, 450, `Item ${i} exceeds 450 chars`);
    }
  });
  ...
  await test('T1.26: Three-word query ("لا يؤمن أحدكم") executes in < 3.0ms', async () => {
    ...
    console.log(`     ${C.gray}Avg latency for "لا يؤمن أحدكم": ${avg.toFixed(3)}ms${C.reset}`);
    assertLessOrEqual(avg, 3.0, `Avg query time ${avg}ms exceeded 3.0ms`);
  });

  await test('T1.27: 95th percentile latency across 20 distinct queries is < 5.0ms', async () => {
    ...
    assertLessOrEqual(p95, 8.0, `P95 latency ${p95}ms exceeded 8.0ms`);
  });
  ...
  await test('T1.39: searchHadithsInBook filters hadiths by chapterId without loading all books', async () => {
    ...
    assertOk(data, 'Failed to load bukhari.json');
    const filtered = searchHadithsInBook(data.hadiths, '', 1);
    assertOk(filtered.length >= 1, 'Expected at least 1 hadith in chapter 1');
    assertOk(filtered.every((h) => h.chapterId === 1), 'All hadiths should belong to chapter 1');
  });
  ...
  await test('T1.41: Memory footprint remains flat (< 30MB heap delta) loading micro-index', () => {
    ...
    assertOk(memBefore < 150 * 1024 * 1024, 'Heap usage abnormally high');
  });
  ...
  await test('T2.13: maxResults = 0 returns empty array []', async () => {
    const res = await searchAcrossAllBooks('النيات', 0);
    assertEqual(res.length, 0);
  });

  await test('T2.14: maxResults = 1 returns exactly 1 top-ranked item', async () => {
    const res = await searchAcrossAllBooks('النيات', 1);
  ...
  await test('T2.16: Negative maxResults handles safely', async () => {
    const res = await searchAcrossAllBooks('النيات', -5);
    assertOk(Array.isArray(res));
  });

  await test('T2.17: Result list never contains duplicate [bookId, hadithId] pairs', async () => {
    const res = await searchAcrossAllBooks('النيات', 50);
    const seen = new Set();
    for (const r of res) {
  ...
  await test('T2.23: Hard latency ceiling: 100% of single-word queries finish in < 10.0ms', async () => {
    const queries = ['الإيمان', 'الإسلام', 'الإحسان', 'التقوى', 'الصبر'];
    for (const q of queries) {
      const t0 = performance.now();
      await searchAcrossAllBooks(q);
      const duration = performance.now() - t0;
      assertLessOrEqual(duration, 10.0, `Query "${q}" took ${duration}ms (> 10.0ms)`);
    }
  });
  ...
  await test('T2.27: 1000 search iterations produce negligible heap delta (< 15MB)', async () => {
    ...
    for (let i = 0; i < 1000; i++) {
      await searchAcrossAllBooks('النيات', 5);
    }
  ...
  await test('T2.35: No null or undefined values in micro-index items', async () => {
    const payload = await getMicroIndexPayload();
    for (let i = 0; i < Math.min(200, payload.items.length); i++) {
  ...
  await test('T2.36: Micro-index book references match valid book IDs', async () => {
    ...
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertOk(validIds.has(f.bookId), `Invalid bookId: ${f.bookId}`);
    }
  });
  ...
  await test('T2.37: Micro-index grade references are valid strings or indices', async () => {
    ...
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertOk(f.grade !== undefined && f.grade !== null && f.grade !== '');
    }
  });
  ...
  await test('T2.38: Every hadith ID is a positive integer >= 1', async () => {
    ...
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertGreaterOrEqual(f.hadithId, 1);
    }
  });
  ...
  await test('T2.39: Every chapter ID is an integer >= 0', async () => {
    ...
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertGreaterOrEqual(f.chapterId, 0);
    }
  });
  ...
  await test('T2.40: Every text preview is a non-empty string', async () => {
    ...
    for (let i = 0; i < Math.min(100, payload.items.length); i++) {
      const f = getItemFields(payload.items[i], payload);
      assertOk(typeof f.textPreview === 'string' && f.textPreview.length > 0);
    }
  });
  ...
  await test('T4.1: Scenario 1 - Canonical Hadith Search: "إنما الأعمال بالنيات"', async () => {
    ...
    assertOk(results.length > 0, 'Expected at least 1 result');
    assertEqual(results[0].book.id, 'bukhari');
    assertEqual(results[0].hadith.idInBook, 1);
  ...
  await test('T4.4: Scenario 4 - Keystroke Simulation: 100 Rapid Interactive Searches (< 250ms total)', async () => {
    const keystrokes = [
