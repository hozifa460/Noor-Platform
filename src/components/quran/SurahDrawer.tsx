...
import { ALL_SURAHS, type SurahMeta } from '@/lib/quran';
...
export function SurahDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex justify-start animate-in fade-in duration-200">
      <div className="w-full max-w-sm sm:max-w-md h-full bg-card border-l border-border flex flex-col shadow-2xl animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 divide-y divide-border/40">
          {filteredSurahs.map((surah) => {
            const isSelected = surah.number === activeSurah.number;
            return (
              <button
                key={surah.number}
                onClick={() => {
