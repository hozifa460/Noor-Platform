...
export default function DownloadClient() {
  ...
  useEffect(() => {
    ...
    if (videoId && videoId !== 'index') fetchInfo();
  ...
  const services = [
    {
      id: 'cobalt',
      label: 'Cobalt Tools (مفتوح المصدر وخالٍ من الإعلانات)',
      url: `https://cobalt.tools/?u=${encodeURIComponent(youtubeUrl)}`,
      quality: 'جميع الجودات حتى 4K',
      type: 'video',
      icon: Download,
      color: 'from-sky-500 to-indigo-600',
      description: 'أداة مفتوحة المصدر عالية الجودة وبدون أي إعلانات',
    },
  ];
  ...
  if (error || videoId === 'index') {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center">
            <AlertCircle className="size-12 mx-auto text-destructive mb-4" />
            <p className="text-muted-foreground mb-4">{error || 'يرجى فتح الصفحة من تطبيق نور لتحميل فيديو'}</p>
