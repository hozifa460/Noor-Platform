 ...
 * with authentic hierarchical Table of Contents, physical pagination, and poetry formatting.
 */
async function loadOpenItiDynamicEBook(bookId: string): Promise<EBookMetaResponse | null> {
  ...
  try {
    // Use our proxy API which handles extension variants (.completed, .mARkdown, etc.)
    ...
    for (const rawLine of rawLines) {
      ...
      // Strip inline pagination & footnote noise
      line = line.replace(/PageV\d+P\d+/g, '').replace(/ms\d+/g, '').replace(/\(¬\d+\)/g, '').trim();
      if (!line) continue;

      // 2. Heading Detection (Major divisions, babs, fasls, poems)
      ...
      // 3. Poetry Line (e.g. % صدر % عجز % 12 or صدر %~% عجز)
      ...
      // Default appended line
      currentParagraph = currentParagraph ? `${currentParagraph} ${line}` : line;

      // Soft subdivision ceiling for giant unstructured books (> 90 paragraphs without any heading)
    ...
    const title = bookItem?.title || 'مصنف تراثي';
    const author = bookItem?.sheikhName || 'من أئمة التراث';
    ...
    console.warn(`[book-text-engine] Failed to dynamically load OpenITI book ${bookId}:`, err);
    return null;
  }
}
