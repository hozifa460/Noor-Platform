...
import type { AdhkarCategory, DhikrItem } from '../types';
...
export function AdhkarHubView() {
  ...
  // Stop audio and release resources on unmount
  useEffect(() => {
    return () => {
      if (audioRef.current) {
  ...
  // Handle Audio toggle
  const handleToggleAudio = useCallback((item: DhikrItem) => {
    if (!item) return;
  ...
  // Copy dhikr text
  const handleCopy = useCallback((item: DhikrItem) => {
    if (!item) return;
